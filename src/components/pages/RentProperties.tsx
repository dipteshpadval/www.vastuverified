...
const RentProperties: React.FC = () => {
  ...
  // Filter properties for rent transaction type from API data
  const rentProperties = state.properties.filter(property => property.transactionType === 'rent')
  ...
  const properties = state.filteredProperties.length > 0 
    ? state.filteredProperties.filter(p => p.transactionType === 'rent')
    : rentProperties
